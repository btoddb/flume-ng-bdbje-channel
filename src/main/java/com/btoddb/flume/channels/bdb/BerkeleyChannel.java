...
import com.btoddb.utils.JmxStatsHelper;
...
public class BerkeleyChannel extends BasicChannelSemantics implements BerkeleyChannelMXBean {
    ...
    public void configure(Context context) {
        ...
        if (null != context) {
            dataDir = new File(context.getString("dataDir", DEFAULT_DB_PATH));
        ...
        if (dataDir.exists()) {
            if (!dataDir.isDirectory()) {
                throw new ChannelException("dataDir, " + dataDir.getAbsolutePath()
                        + ", is not a directory - cannot continue");
            }
        }
        else if (!dataDir.mkdir()) {
            throw new ChannelException("dataDir, " + dataDir.getAbsolutePath() + ", is not a directory - cannot continue");
        }
